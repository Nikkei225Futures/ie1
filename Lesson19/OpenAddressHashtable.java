public class OpenAddressHashtable {
    ...
    public boolean put(AddressData anAddressData) {
        ...
        if (null == anAddressData) {
            // 格納対象のデータはnull なのでfalse を返し処理を抜ける
            return false;
        }
        ...
        if (null == this.table[hashCode] || this.removedData == this.table[hashCode]) {
            // ハッシュ値の場所に挿入可能なので挿入してtrue を返し// 処理を抜ける
        ...
        for (int count = 0; count < limit; count++) {
            ...
            // 再ハッシュ値の場所に挿入可能か調べる
            if (null == this.table[hashCode] || this.removedData == this.table[hashCode]) {
                // 再ハッシュ値の場所に挿入可能なので挿入して// true を返し処理を抜ける
                this.table[hashCode] = anAddressData;return true;
            }
        }
        return false;
    }
    ...
    private int calculateHashCodeAgain(String aKey, int aHashCode) {
        ...
        System.err.println("rehash: " + aKey + " " + aHashCode + " -> " + rehashCode);
        return rehashCode;
    }
}